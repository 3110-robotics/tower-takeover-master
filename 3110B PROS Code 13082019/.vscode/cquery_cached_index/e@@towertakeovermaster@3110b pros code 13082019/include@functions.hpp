...
namespace acontrol{
  class aselector{
    private:
      lv_style_t Style;
      uint8_t stylergb[3][3];
      void style_set();

      lv_obj_t * myButton;
      lv_style_t bStyles[2];
      lv_align_t balign;
      uint8_t mybLocation[3];
      void button_creater();

      lv_obj_t * bLabelObjects[2];
      const char * label;
      void button_label_creater();
    ...
    public:
      ...
      void button_create(lv_obj_t * myButton, lv_style_t stylerel, lv_style_t stylepr, lv_align_t align, uint8_t place, uint8_t myWidth, uint8_t myHeight);
  ...
  class autons{
    private:
      ...
      void PIDTuner();
  public:
    void autonTriggerer(auton_type Type);
  };
  class pid{
    public:
      int dT;
