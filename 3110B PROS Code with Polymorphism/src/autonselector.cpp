...
lv_res_t acontrol::aselector::btn_click_action(lv_obj_t * a){
  ...
  switch(id){ //triggers a 4 way switch
    case 0: count+=1; (count>10 ? count=0:count); sprintf(buffer, "%s", autonModes[count]); lv_label_set_text(autonLabel, buffer); //increases count and updates the auton Mode label
    case 1: count-=1; (count<0 ? count=10:count); sprintf(buffer, "%s", autonModes[count]); lv_label_set_text(autonLabel, buffer); //decreases count and updates the auton Mode label
