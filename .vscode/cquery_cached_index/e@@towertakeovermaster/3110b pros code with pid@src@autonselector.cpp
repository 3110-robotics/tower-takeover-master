...
void acontrol::aselector::style_setter(lv_style_t style, uint8_t r1, uint8_t b1, uint8_t g1, uint8_t r2, uint8_t g2, uint8_t b2, uint8_t r3, uint8_t g3, uint8_t b3){
  ...
  Style.body.main_color = LV_COLOR_MAKE(r1, g1, b1); // sets the colours of the style
  Style.body.grad_color = LV_COLOR_MAKE(r2, g2, b2);
  Style.body.radius = 0;
  Style.text.color = LV_COLOR_MAKE(r3, g3, b3);
}
...
lv_res_t acontrol::aselector::btn_click_action(lv_obj_t * a){
  ...
  switch(id){ //triggers a 4 way switch
    case 0: count+=1; (count>9 ? count=0:count); sprintf(buffer, "%s", autonModes[count]); lv_label_set_text(autonLabel, buffer); //increases count and updates the auton Mode label
    case 1: count-=1; (count<0 ? count=8:count); sprintf(buffer, "%s", autonModes[count]); lv_label_set_text(autonLabel, buffer); //decreases count and updates the auton Mode label
